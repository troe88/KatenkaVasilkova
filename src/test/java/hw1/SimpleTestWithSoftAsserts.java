...
public class SimpleTestWithSoftAsserts {

    @Test
    public void simpleTestWithSoftAsserts() {
        ...
        softAssert.assertTrue(navBarElements.stream().allMatch(WebElement::isDisplayed));
        softAssert.assertEquals(actualNavBarElementsTexts.toArray(), expectedNavBarElementsTexts.toArray());
        ...
        softAssert.assertTrue(benefitIcons.stream().allMatch(WebElement::isDisplayed));
        //OR the same assertion joined:
        softAssert.assertTrue(benefitIcons.size() == 4 && benefitIcons.stream().allMatch(WebElement::isDisplayed));
        ...
        //splited assertion:
        softAssert.assertEquals(underBenefitIconsElements.size(), 4);
        softAssert.assertTrue(underBenefitIconsElements.stream().allMatch(WebElement::isDisplayed));
        softAssert.assertEquals(actualUnderBenefitIconsElementsTexts.toArray(), expectedUnderBenefitIconsElementsTexts.toArray());
        //OR the same assertion joined:
        softAssert.assertTrue(
        ...
        //splited assertion:
        softAssert.assertTrue(mainHeadersElements.stream().allMatch(WebElement::isDisplayed));
        softAssert.assertEquals(actualMainHeadersElementsTexts.toArray(), expectedMainHeadersElementsTexts.toArray());
        //OR the same assertion joined:
        softAssert.assertTrue(
        ...
        softAssert.assertTrue(LocationHelper.whereIsWebElementInTheWindow(iframe, driver.manage().window()).toString()
                .contains(LocationHelper.WhereIsWebElement.CENTER.toString()));

        //11    Switch to the iframe and check that there is Epam logo in the left top conner of iframe
        driver.switchTo().frame(iframe);
        softAssert.assertEquals(LocationHelper.whereIsWebElementInTheWebElement(
        ...
        List<WebElement> mainContainerBlocks = driver.findElements(By.cssSelector("body > div > div"));
        //check if there are more than one block and one of them is located on the left
        softAssert.assertTrue(
                mainContainerBlocks.size() > 1
                        && mainContainerBlocks.stream().anyMatch(e -> e.getLocation().x == 0 && e.isDisplayed())
        );

        //16    Assert that there is Footer
        softAssert.assertTrue(driver.findElement(By.cssSelector("footer")).isDisplayed());

        //summarize soft asserts
